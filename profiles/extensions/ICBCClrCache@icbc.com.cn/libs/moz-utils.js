...
function runOnLoadPage(win, callback) {
  // Listen for one load event before checking the window type
...
function runOnPage(callback, url) {
  // Wrap the callback in a function that ignores failures
  function watcher(window) {
    try {
      callback(window);
  ...
  function _getDoc(aBrowser)
  {
    let tabs = aBrowser.tabs;
    for(let i = 0; i < tabs.length; i++)
    {
      let _doc = tabs[i].linkedBrowser.webNavigation.document;
      if(_doc)
        return _doc;  
    } 
    return null; 
  ...
  while (browserWindows.hasMoreElements()) {
    // Only run the watcher immediately if the browser is completely loaded
    let browserWindow = browserWindows.getNext();
    let br = browserWindow.document.getElementById("content");
    let tabs = br.tabs;
    for(let i = 0; i < tabs.length; i++)
    {
      let doc = tabs[i].linkedBrowser.webNavigation.document;
      doc = XPCNativeWrapper.unwrap(doc);
      if(doc && doc.defaultView && isMatchURL(doc.baseURI, url)){
        if (doc.readyState == "complete")
          watcher(doc.defaultView);
        else
          runOnLoadPage(doc.defaultView, watcher);
      }
    } 
  }
}
function watchPage(callback, url) {
  function watcher(window) {
    try {
      callback(window);
    }
  ...
  // Watch for new browser page opening then wait for it to load
  let pageObserver = {
      observe: function(subject, topic, data){
        let win = XPCNativeWrapper.unwrap(subject);
        let top = win ? win.top : false;
        if(win && win == top && isMatchURL(win.document.baseURI, url))
          runOnLoadPage(win, watcher);
      }
  };
  Services.obs.addObserver(pageObserver,"content-document-global-created",false);
  // Add functionality to existing page
  runOnPage(callback, url);

  // Make sure to stop watching for windows if we're unloading
  unload(function() Services.obs.removeObserver(pageObserver,"content-document-global-created"));
}
/**
